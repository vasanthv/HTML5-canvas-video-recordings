...
window.WhiteboardUi = {
	...
	addListeners: function() {
		WhiteboardUi.getElement('button_pencil').mousedown(function() {
			Whiteboard.setStrokeStyle(Whiteboard.context.strokeStyle);
			WhiteboardUi.activatePencil();
		});
		WhiteboardUi.getElement('button_arrow').mousedown(function() {
			Whiteboard.setStrokeStyle(Whiteboard.context.strokeStyle);
			WhiteboardUi.activateArrow();
		});
		...
		WhiteboardUi.getElement('button_color_red').mousedown(function() {
			Whiteboard.setStrokeStyle('#ff0000');
			if($('.controlicon.active').length < 1){
				WhiteboardUi.activatePencil();
				$('#button_pencil').addClass('active');
			}
		});
		WhiteboardUi.getElement('button_color_black').mousedown(function() {
			Whiteboard.setStrokeStyle('#000000');
			if($('.controlicon.active').length < 1){
				WhiteboardUi.activatePencil();
				$('#button_pencil').addClass('active');
			}
		});
		WhiteboardUi.getElement('button_color_white').mousedown(function() {
			Whiteboard.setStrokeStyle('#ffffff');
			if($('.controlicon.active').length < 1){
				WhiteboardUi.activatePencil();
				$('#button_pencil').addClass('active');
			}
		});
		WhiteboardUi.getElement('button_color_blue').mousedown(function() {
			Whiteboard.setStrokeStyle('#0060FF');
			if($('.controlicon.active').length < 1){
				WhiteboardUi.activatePencil();
				$('#button_pencil').addClass('active');
			}
		});
		WhiteboardUi.getElement('button_eraser').mousedown(function(){
			WhiteboardUi.activateEraser
			Whiteboard.setStrokeStyle('#ffffff');
			WhiteboardUi.activatePencil();
		...
		WhiteboardUi.getElement('button_text').mousedown(function() {
			Whiteboard.setStrokeStyle(WhiteboardUi.getElement('input_color').attr("value"));
	...
	getY: function(event) {
	    ...
	    //return canvasy;
	    //added by vasanth
	    return (canvasy+$(document).scrollTop());
	},
	...
	activateText: function(event) {
		...
		WhiteboardUi.canvasElement.bind("click", WhiteboardUi.beginTextDraw);
		WhiteboardUi.canvasElement.addClass(WhiteboardUi.getElementName('text_active'));
	},

	beginTextDraw: function(event) {
		//alert(WhiteboardUi.getX(event));
		//alert(WhiteboardUi.getY(event));
		//alert(Whiteboard.context.strokeStyle);
		$('#add_text').css('top', WhiteboardUi.getY(event));
		$('#add_text').css('left', WhiteboardUi.getX(event));
		$('#add_text').css('color', Whiteboard.context.strokeStyle);
		$('#add_text').attr('data-x', WhiteboardUi.getX(event));
		$('#add_text').attr('data-y', WhiteboardUi.getY(event));
		$('#add_text').attr('data-color', Whiteboard.context.strokeStyle);
		$('#add_text').show();
		$('#add_text').focus();
	 ...
	 * this action
	 */
	activateArrow: function(event) {
		WhiteboardUi.changeTool();
		WhiteboardUi.canvasElement.bind("mousedown", WhiteboardUi.beginArrow);
	},

	/**
	 * Begins arrow drawing at the current point. This also
	 ...
	 * this action
	 */
	beginArrow: function(event) {
		Whiteboard.beginShape(WhiteboardUi.getX(event), WhiteboardUi.getY(event));
	    WhiteboardUi.canvasElement.bind("mousemove", function(event) {
	        Whiteboard.drawArrow(WhiteboardUi.getX(event), WhiteboardUi.getY(event));
	    });
		WhiteboardUi.canvasElement.bind("mouseup", WhiteboardUi.endArrow);
	},
