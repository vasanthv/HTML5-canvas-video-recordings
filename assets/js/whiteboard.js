...
window.Whiteboard = {
    ...
    execute: function(wbevent, firstexecute) {
        ...
        if(type === "beginpath") {
            ...
            this.context.moveTo(wbevent.coordinates[0], wbevent.coordinates[1]);
            this.context.stroke();
        } else if(type === "beginshape") {
	        this.context.save();
	        this.context.beginPath();
        } else if (type === "drawpathtopoint") {
            this.context.lineTo(wbevent.coordinates[0], wbevent.coordinates[1]);
            this.context.stroke();
			...
			var length = 8;var width = 8;
			slopy = Math.atan2((sy - ey), (sx - ex));
		    var cosy = Math.cos(slopy);
		    var siny = Math.sin(slopy);

			this.context.moveTo(ex, ey);
			this.context.lineTo(ex + (length * cosy - ( width/ 2.0 * siny)), ey + (length * siny + (width/2 * cosy)));
		    this.context.lineTo(ex + (length * cosy + width/2.0 * siny), ey - (width/2 * cosy - length * siny));
			this.context.closePath();
	        this.context.stroke();
    ...
    /**
     * Draws a arrow that has opposite corner to the
    ...
    */
    redraw: function(type) {
		...
		if((redrawEvents.length > 3 && type=='undo') || type=='redo'){
			this.events = [];
		    //Whiteboard.context.clearRect(0,0,Whiteboard.canvas.width,Whiteboard.canvas.height);
	        ...
	        if(type=='redo'){
	        	var redoevents = this.extraevents;
	            this.execute(redoevents[0]);
	        }
		}else{
			var imageObj = new Image();
			var canvas_ratio = 960/540;
			imageObj.src = $('#canvas').attr('data-url');
			imageObj.onload = function() {
				var rw = imageObj.width / 960; // width and height are maximum thumbnail's bounds
				var rh = imageObj.height / 540;
				...
				Whiteboard.context.fillRect(0,0,960,540);
				Whiteboard.context.drawImage(imageObj, 0, 0, neww, newh);
			};
		}
    },
    ...
    */
    undo: function() {
        this.extraevents.unshift(this.events.pop());
    ...
    /**
     * This redrawas the undone action
    */
    redo: function() {
		this.extraevents.shift();
        //console.log(reverseEvent.type);
        this.redraw('redo');
    }
